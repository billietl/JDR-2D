...
\maketitle
\vspace{475pt}
\includegraphics[scale=0.06]{./licence.png}
\thispagestyle{empty}
\clearpage
...
\item
  Adapter : r\'earragner, transformer, et cr\'eer \`a partir de cette oeuvre.
\end{itemize}
peu importe votre but, m\^eme commercial.
...
\item
  Propagation : Si vous r\'earragnez, transformez, et cr\'eez \`a partir de ce oeuvre, vous devez distribuer votre contribution sous la m\^eme licence.
\item
...
Je vous demande juste, en cas de contribution, d'ajouter votre nom (ou pseudonyme) ainsi que la date \`a laquelle vous avez contribu\'e dans la liste ci-dessous.
...
C'est tr\`es simple : je voulais faire jouer un sc\'enario qui se d\'eroulais dans dans l'univers du manga Fullmetal Alchemist, mais ne trouvais pas de syst\`eme qui me convenais.
Chaque syst\`eme avait au moins un des probl\`emes suivant :
\begin{itemize}
\item
  Les diff\'erents r\'esultats obtenable sont \'equiprobables.
...
\item
  La feuille de personnage est beaucoup trop cryptique.
...
\item
  ...
  Je n'aime pas l'id\'ee.
\end{itemize}

Je voulais un syst\`eme o\`u il est beaucoup plus probable d'obtenir un r\'esultat moyen qu'avoir un r\'esultat hors norme.
Je voulais un syst\`eme facile \`a apr\'ehender, aussi bien par les joueurs confirm\'ee que par les n\'eophytes.
Je voulais un syst\`eme privil\'egiant le roleplay \`a l'application b\^ete de r\`egles.
...
Pour cela, j'ai mis au point ce syst\`eme.
\clearpage
...
Bien que la profession puisse \^etre principalement cosm\'etique, elle permet n\'eanmoins de d\'efinir ce que personnage sais et sais faire.
...
Par contre, savoir si, dans un univers contemporain, un \'electronicien aie des comp\'etences en informatique sera beaucoup plus subtil.
\subsection{Les caract\'eristiques}
Les caract\'eristiques permettent de sp\'ecifier les pr\'edisposition qu'ont les personnage par rapport aux t\^aches.
...
Le social intervient lorsque le personage interagis avec les PNJs, que ce soit de mani\`ere active (discussion, ...) ou passive (impression, ...).
...
\item
  ...
  Ce sera donc au joueur de les faire impacter dans sa mani\`ere de jouer, et au ma\^itre du jeu de faire r\'eagir les PNJ en consc\'equence.
  Par exemple, un personnage \textit{x\'enophobe} aura du mal \`a discuter normalement avec un personnage \'etranger.
\item
  ...
  Par exemple, un personnage ayant une \textit{dette envers la mafia} sera importun\'e pendant la partie, et ne voudra pas que les autrs joueurs d\'ecouvrent de quoi il en est.
\end{itemize}
\subsection{Les comp\'etences}
Les comp\'etences d\'efinissent \`a la fois les comp\'etences th\'eoriques et pratiques du personnage.
...
Le choix des avantages et d\'esavantages disponible est \`a la discretion du MJ, en fonction de l'univers, bien entendu.
...
\'Etant donn\'e que la liste des comp\'etences disponibles dans l'univers d\'ependra beaucoup du MJ, je lui conseille de garder une version des feuilles de personnage pour lui.
Ensuite, le MJ choisira parmis la liste des comp\'etences du personnage celle qui semble la plus appropri\'ee.
...
Si le personnage \`a une comp\'etence th\'eorique associ\'ee (ici, \textit{architecture} ou \textit{ma\c connerie}), le MJ fera le jet associ\'e et le but sera atteint si le r\'esultat est sup\'erieur ou \'egal \`a z\'ero.
...
S'il fait entre -1 et -3, Bob se receptionnera mal et perdra son élan.
...
Le MJ doit annoncer aux joueurs les blessures que subissent les personnages au fur et a mesure du jeu.
Il appartient au MJ de garder \`a l'esprit l'\'etat des presonnages afin de les r\'epercuter sur les niveaux de difficult\'es.
\end{document}
